...
int main() {
	...
	GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "PREVIO 3 Proyecciones y transformaciones basicas    ALEJANDRO MARTINEZ JIMENEZ", nullptr, nullptr);
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate( model, 50.0f, glm::vec3( 0.0f, 10.0f, 0.0f ) ); // use to compare orthographic and perspective projection
		...
		model = glm::rotate(model, 50.0f, glm::vec3(0.0f, 10.0f, 1.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(2.0f, 2.0f, 2.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
